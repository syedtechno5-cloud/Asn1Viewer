...
class GrammarManager:
    ...
    def _normalize_and_add(self, tag_id: str, name: str):
        """
        Normalise a tag ID string and add it to the grammar dictionary.

        Accepted formats:
          - Hex with prefix:   "0x4F", "0x5F20", "0X9F26"
          - Hex without prefix: "4F", "5F20", "9F26"
          - Decimal:            "79"

        Keys are stored as "0x{HEX}" with uppercase hex digits and the
        minimum number of nibbles needed (always even, so single bytes
        are "0xNN" and multi-byte tags are "0xNNNN" etc.).
        This matches the format produced by ASN1Tag.get_grammar_key().
        """
        try:
            raw = tag_id.strip()
            if raw.lower().startswith('0x'):
                ...
                # Determine whether it looks like hex or decimal
                try:
                    hex_digits = raw
                    int(raw, 16)          # validate
                except ValueError:
                    ...
                    value = int(raw, 10)
                    hex_digits = f"{value:02X}"

            # Pad to an even number of nibbles so bytes are aligned
            if len(hex_digits) % 2:
                hex_digits = '0' + hex_digits

            key = "0x" + hex_digits.upper()
